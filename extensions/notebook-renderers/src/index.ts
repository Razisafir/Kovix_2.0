...
export const activate: ActivationFunction<void> = (ctx) => {
	...
	#container div.output .scrollable.scrollbar-visible {
		border-color: var(--construct-editorWidget-border);
	}
	...
	#container div.output .scrollable.more-above {
		box-shadow: var(--construct-scrollbar-shadow) 0 6px 6px -6px inset
	}
	...
	#container .error-output-actions li {
		...
		display: inline-flex;
		cursor: pointer;
		border: solid 1px var(--construct-notebook-cellToolbarSeparator);
	}
	#container .error-output-actions li.hover {
		background-color: var(--construct-toolbar-hoverBackground);
	}
	...
	#container .error-output-actions li a {
		color: var(--construct-foreground);

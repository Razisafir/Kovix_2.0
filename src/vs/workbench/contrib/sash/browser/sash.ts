...
export class SashSettingsController implements IWorkbenchContribution, IDisposable {
	...
	private onDidChangeSize(): void {
		...
		this.layoutService.mainContainer.style.setProperty('--construct-sash-size', size + 'px');
		this.layoutService.mainContainer.style.setProperty('--construct-sash-hover-size', hoverSize + 'px');

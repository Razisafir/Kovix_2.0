...
export class UniversalMemoryService extends Disposable implements IUniversalMemoryService {
	...
	constructor(
		...
		const kovixDir = URI.joinPath(environmentService.userHome, '.kovix');
	...
	async searchMemories(query: IUniversalMemoryQuery): Promise<IUniversalMemoryEntry[]> {
		...
		if (queryTerms.length > 0) {
			results = results.map(m => {
				...
				const searchText = `${contentLower} ${tagsLower} ${m.projectName.toLowerCase()}`;
		...
		return new Promise((resolve) => {
			this._saveTimeout = setTimeout(async () => {
				try {
					...
					await this.fileService.writeFile(this._storagePath, new TextEncoder().encode(data));
				} catch (error) {

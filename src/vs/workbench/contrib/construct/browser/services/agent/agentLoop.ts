 ...
 *  Orchestrates LLM calls with tool execution, streaming, and memory injection.
 *  Implements Plan/Act approval flow with 15-round limit.
...
import { IAnthropicProvider, IAnthropicMessage, IAnthropicContentBlock, IAnthropicTool, StreamEvent } from '../../../../../../platform/construct/common/llm/anthropicProvider.js';
...
export class AgentLoopService extends Disposable implements IAgentLoop {
                                ...
                                for await (const event of stream) {
                                        ...
                                        switch (event.type) {
                                                ...
                                                case 'tool_end': {
                                                        ...
                                                        // Execute the tool (read-only during planning)
                                ...
                                if (hadToolCalls && stopReason === 'tool_use') {
                                        ...
                                        for (const block of assistantContent) {
                                                if (block.type === 'tool_use') {
                                                        const toolResult = await this.executeTool(block.name, block.input, true);
                                                        toolResults.push({
                                                                type: 'tool_result',
        ...
        async *run(task: string, signal?: AbortSignal): AsyncGenerator<AgentLoopEvent> {
                ...
                try {
                        ...
                        while (roundCount < MAX_ROUNDS) {
                                ...
                                for await (const event of stream) {
                                        if (signal?.aborted) {
                                                yield { type: 'error', text: '⏹ Stopped by user', recoverable: false };
                                        ...
                                        switch (event.type) {
                                                ...
                                                case 'tool_end': {
                                                        ...
                                                        if ((event.toolName === 'write_file' || event.toolName === 'edit_file') && success) {
                                                                filePath = (event.toolInput as any)?.path ?? '';
                                                                if (filePath) {
        ...
        private buildSystemPrompt(task: string, planningOnly: boolean): string {
                ...
                return `You are CONSTRUCT, an expert AI coding assistant.

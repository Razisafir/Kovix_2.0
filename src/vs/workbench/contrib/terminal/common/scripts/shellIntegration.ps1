...
# Set always on key handlers which map to default Kovix IDE keybindings
...
function Set-MappedKeyHandlers {
	...
	if ($env:VSCODE_SUGGEST -eq '1' -and $PSVersionTable.PSVersion -ge "7.0") {
		...
		# Kovix IDE send completions request (may override Ctrl+Spacebar)
		Set-PSReadLineKeyHandler -Chord 'F12,e' -ScriptBlock {
			Send-Completions
		}

		# Kovix IDE send global completions request

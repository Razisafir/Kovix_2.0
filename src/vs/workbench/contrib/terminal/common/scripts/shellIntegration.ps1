...
# Set always on key handlers which map to default CONSTRUCT IDE keybindings
...
function Set-MappedKeyHandlers {
	...
	if ($env:VSCODE_SUGGEST -eq '1' -and $PSVersionTable.PSVersion -ge "7.0") {
		...
		# CONSTRUCT IDE send completions request (may override Ctrl+Spacebar)
		Set-PSReadLineKeyHandler -Chord 'F12,e' -ScriptBlock {
			Send-Completions
		}

		# CONSTRUCT IDE send global completions request

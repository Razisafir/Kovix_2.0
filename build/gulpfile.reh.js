...
function getNodeVersion() {
        ...
        const nodeVersion = pkgJson.config?.remoteNodeVersion;
        if (!nodeVersion) {
                throw new Error('package.json config.remoteNodeVersion is missing');
        }
        // internalNodeVersion was previously read from remote/.npmrc ms_build_id,
        // but npm 11.13.0 deprecated that key. Retain for backward compatibility.
        const internalNodeVersion = pkgJson.config?.remoteNodeMsBuildId ?? '';
